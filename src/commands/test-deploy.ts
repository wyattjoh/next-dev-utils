...
export async function testDeployCommand(options: Options) {
  ...
  if (!existsSync(testFile)) {
    throw new Error(
      `The test file ${
        options["test-file"]
      } does not exist. Please specify a valid test file.`,
    );
  }
  ...
  if (!testFile.startsWith(e2eDir + path.sep)) {
    ...
    throw new Error(
      `The test file ${
        options["test-file"]
      } is not in a test/e2e directory. This can only be used with e2e tests.`,
    );
  }
  ...
  if (!/\.test\.(js|ts)$/.test(testFile)) {
    throw new Error(
      `The test file ${
        options["test-file"]
      } does not match the pattern *.test.{js,ts}. Please specify a valid test file.`,
    );
  }
